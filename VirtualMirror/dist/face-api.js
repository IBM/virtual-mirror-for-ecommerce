...
/*Global variables for AR*/	
var x1=0; // Temp variable
var y1=0; // Temp variable
var pix=37.7952755906; // 1cm = 37.7952755906px
var earLenghtInCentimeter=4; // Average size of an ear.
var noseLenghtInCentimeter=2;
var dist=0; // size of the ear/nose
	
	function drawMyNose(ctx, points, isClosed, img, height_jewellery, width_jewellery) {
        if (isClosed === void 0) { isClosed = false; }
        ctx.beginPath();
		//console.log(points.slice(1))
        points.slice(1).forEach(function (_a, prevIdx) {
            ...
            var from = points[prevIdx];
						...
						var noseLenghtInPixel=earLenghtInCentimeter*pix;
						var noseRingLenghtInPixel=height_jewellery*pix;
						var noseRingWidthInPixel=width_jewellery*pix;
						
				if (prevIdx == 4){x1=from.x; y1=from.y;	}
				if (prevIdx == 7){
								console.log(x)
								const dis= euclideanDistance([x1,y1], [x,y])
								dist = dis
								const w=(noseRingWidthInPixel*dist)/noseLenghtInPixel;
								const l=(noseRingLenghtInPixel*w)/noseRingWidthInPixel;
								ctx.drawImage(img, x+4, y-15, w, l);
					}
	...
	function drawMyEars(ctx, points, isClosed, img, height_jewellery, width_jewellery) {
        if (isClosed === void 0) { isClosed = false; }
        ctx.beginPath();
		//console.log(points.slice(1))
        points.slice(1).forEach(function (_a, prevIdx) {
            ...
            var from = points[prevIdx];
				...
				var earLenghtInPixel=earLenghtInCentimeter*pix;
				var earRingLenghtInPixel=height_jewellery*pix;
				var earRingWidthInPixel=width_jewellery*pix;
				
				

				if (prevIdx == 0){x1=from.x; y1=from.y;	}
				if (prevIdx == 1){
								//console.log(x1);console.log(y1);
              			
					var x2=x; 
					var y2=y; 
					            //console.log(x2);console.log(y2)
					const dis= euclideanDistance([x1,y1], [x2,y2])
					dist = dis
					
					const l=(earRingLenghtInPixel*dis)/earLenghtInPixel;
					//console.log(l)
					const w=(earRingWidthInPixel*dis)/earLenghtInPixel;
					//console.log(w)
					//console.log(dist)
					            //ctx.moveTo(x+4, y-8);
								//ctx.lineTo(x+4, y-4);
								ctx.drawImage(img, x-12, y, w, l);
					}
					if (prevIdx == 13){
					console.log(dist)
					            //ctx.moveTo(x+4, y-8);
								//ctx.lineTo(x+4, y-4);
								const l=(earRingLenghtInPixel*dist)/earLenghtInPixel;
								const w=(earRingWidthInPixel*l)/earRingLenghtInPixel;
								ctx.drawImage(img, x+1, y, w, l);
					}
    ...
    function drawLandmarks(canvasArg, faceLandmarks, options) {
        ...
        faceLandmarksArray.forEach(function (landmarks) {
            if (drawLines && landmarks instanceof FaceLandmarks68) {
				...
				//drawMyContour(ctx, landmarks.getNose());
	    ...
	    function drawJewellery(canvasArg, faceLandmarks, options, img, height_jewellery, width_jewellery) {
        var canvas = resolveInput(canvasArg);
        if (!(canvas instanceof env.getEnv().Canvas)) {
            throw new Error('drawLandmarks - expected canvas to be of type: HTMLCanvasElement');
        }
        ...
        faceLandmarksArray.forEach(function (landmarks) {
            if (drawLines && landmarks instanceof FaceLandmarks68) {
                ...
                ctx.lineWidth = lineWidth;
                //drawContour(ctx, landmarks.getJawOutline());
				//drawMyEars(ctx, landmarks.getJawOutline(), false, img);
                //drawContour(ctx, landmarks.getLeftEyeBrow());
                //drawContour(ctx, landmarks.getRightEyeBrow());
                //drawContour(ctx, landmarks.getNose());
				drawMyNose(ctx, landmarks.getNose(),false, img, height_jewellery, width_jewellery);
                //drawContour(ctx, landmarks.getLeftEye(), true);
                //drawContour(ctx, landmarks.getRightEye(), true);
                //drawContour(ctx, landmarks.getMouth(), true);
                return;
		...
		function drawJewelleryWebcam(canvasArg, faceLandmarks, options, nimg, eimg) {
        var canvas = resolveInput(canvasArg);
        if (!(canvas instanceof env.getEnv().Canvas)) {
            throw new Error('drawLandmarks - expected canvas to be of type: HTMLCanvasElement');
        }
        ...
        faceLandmarksArray.forEach(function (landmarks) {
            if (drawLines && landmarks instanceof FaceLandmarks68) {
                ...
                ctx.lineWidth = lineWidth;
                //drawContour(ctx, landmarks.getJawOutline());
				drawMyEars(ctx, landmarks.getJawOutline(), false, eimg);
                //drawContour(ctx, landmarks.getLeftEyeBrow());
                //drawContour(ctx, landmarks.getRightEyeBrow());
                //drawContour(ctx, landmarks.getNose());
				drawMyNose(ctx, landmarks.getNose(),false, nimg);
                //drawContour(ctx, landmarks.getLeftEye(), true);
                //drawContour(ctx, landmarks.getRightEye(), true);
                //drawContour(ctx, landmarks.getMouth(), true);
                return;
	    ...
	    function drawJewelleryEars(canvasArg, faceLandmarks, options, img, height_jewellery, width_jewellery) {
        var canvas = resolveInput(canvasArg);
        if (!(canvas instanceof env.getEnv().Canvas)) {
            throw new Error('drawLandmarks - expected canvas to be of type: HTMLCanvasElement');
        }
        ...
        faceLandmarksArray.forEach(function (landmarks) {
            if (drawLines && landmarks instanceof FaceLandmarks68) {
                ...
                ctx.lineWidth = lineWidth;
                //drawContour(ctx, landmarks.getJawOutline());
				drawMyEars(ctx, landmarks.getJawOutline(), false, img, height_jewellery, width_jewellery);
                //drawContour(ctx, landmarks.getLeftEyeBrow());
                //drawContour(ctx, landmarks.getRightEyeBrow());
                //drawContour(ctx, landmarks.getNose());
				//drawMyNose(ctx, landmarks.getNose(),false, img);
                //drawContour(ctx, landmarks.getLeftEye(), true);
                //drawContour(ctx, landmarks.getRightEye(), true);
                //drawContour(ctx, landmarks.getMouth(), true);
                return;
	...
	exports.drawMyNose = drawMyNose;
	exports.drawMyEars = drawMyEars;
    exports.drawLandmarks = drawLandmarks;
	exports.drawJewellery = drawJewellery;
	exports.drawJewelleryWebcam = drawJewelleryWebcam;
	exports.drawJewelleryEars = drawJewelleryEars;
